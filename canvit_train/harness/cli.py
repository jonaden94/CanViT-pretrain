"""Full-fidelity CLI for the unified harness (``python -m canvit_train.harness.run``).
...
line, including the nested trees a hand-rolled argparse could not express:
    ...
    python -m canvit_train.harness.run ade20k --preset probe
    python -m canvit_train.harness.run ade20k --preset joint --rl.use-rl True

**The task config is the source of truth.** :class:`~canvit_train.harness.run.RunSettings`
