...
from .rollout import consumes_full_image, make_random_viewpoints, rollout_cls_tokens
...
def evaluate(clf, cfg: In1kConfig, val_loader, *, device, canvas_grid, amp_ctx, is_foveated,
    ...
    historical trajectory, so the standalone entry point behaves as it always did; the
    harness passes the already-resolved value. ``"policy"`` deploys ``joint``'s scorer by
    argmax, which needs the live canvas and so cannot precompute its viewpoints."""
...
def build_classifier(cfg: In1kConfig, device) -> CanViTForImageClassification:
    """Construct the CanViT classifier for ``cfg.mode`` (shared by the harness task and
    this standalone so the two can't diverge).
...
def train(cfg: In1kConfig) -> None:
    ...
    # Pretrained backbone + mode-dependent head (fresh for the frozen probe, fused DINOv3
    # probe for finetune — see build_classifier).
    clf = build_classifier(cfg, device)
    if cfg.mode == "frozen":
