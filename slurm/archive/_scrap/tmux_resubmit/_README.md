...
```bash
bash slurm/tmux_resubmit/check_and_submit.sh --start --jobs 10
```
This creates a state file (`slurm/canvit_train_state`) with the run name
and job count. No jobs are submitted yet.

**2. Start the watcher in a tmux session:**
```bash
tmux new -s canvit
bash slurm/tmux_resubmit/start_watcher.sh
# Detach: Ctrl+B D
...
```bash
SBATCH_SCRIPT="$REPO/slurm/tmux_resubmit/train_v100_tmux_resubmit.sbatch"       # single GPU
SBATCH_SCRIPT="$REPO/slurm/tmux_resubmit/train_ddp_v100_tmux_resubmit.sbatch"   # 2× GPU DDP
```
