...
| **P1** | Harness seams in pretrain: extract Selector + Task interfaces around the existing loop, `RandomSelector` + `DistillTask` only, everything default-off | **bit-for-bit parity**: pinned-commit diff of losses/metrics vs a reference run (same seed, same data order) over ≥500 steps |
| **P2** | ADE20K task on the stable wrapper (D3) + foveated task-path support (copy canvit_eval's `consumes_full_image` routing) + unified ADE dataset | reproduces specialize's exp22 ADE probe numbers (uniform); foveated probe trains end-to-end (new capability) |
