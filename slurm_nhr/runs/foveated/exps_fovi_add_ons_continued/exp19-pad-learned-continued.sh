#!/bin/bash
# (5) baseline + learned padding (per kernel-cell)
...
RUN_NAME=exp19-pad-learned
ARRAY=0-2%1                                   # TAIL: 70 jobs done (step 286720) -> 3 remaining -> 73 total (299,008 steps, ~300k)
TIME=0-00:45:00
MEM=128G
NGPU=1
...
CFG_LOG_EVERY=512
CFG_NUM_WORKERS=4
EXTRA_ARGS="--model.patcher-name foveated --model.foveated-patcher.resolution 64 --model.foveated-patcher.cart-patch-size 8 --model.foveated-patcher.arch-flag doubleres --model.foveated-patcher.padding learned"
# =================
