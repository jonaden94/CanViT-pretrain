#!/bin/bash
# (8) baseline + learned per-patch output bias
...
RUN_NAME=exp19-cond-bias
ARRAY=0-0%1                                   # CONTINUE: 72 jobs done (step 294912) -> 1 remaining -> 73 total (299,008 steps, ~300k)
TIME=0-00:45:00
MEM=128G
NGPU=1
...
CFG_LOG_EVERY=512
CFG_NUM_WORKERS=4
EXTRA_ARGS="--model.patcher-name foveated --model.foveated-patcher.resolution 64 --model.foveated-patcher.cart-patch-size 8 --model.foveated-patcher.arch-flag doubleres --model.foveated-patcher.conditioning.mode bias"
# =================
