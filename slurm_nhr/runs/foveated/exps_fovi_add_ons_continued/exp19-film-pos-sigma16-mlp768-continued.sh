#!/bin/bash
# (12) FiLM(position, sigma=16) + MLP head [768]
...
RUN_NAME=exp19-film-pos-sigma16-mlp768
ARRAY=0-3%1                                   # TAIL: 69 jobs done (step 282624) -> 4 remaining -> 73 total (299,008 steps, ~300k)
TIME=0-00:45:00
MEM=128G
NGPU=1
...
CFG_LOG_EVERY=512
CFG_NUM_WORKERS=4
EXTRA_ARGS="--model.patcher-name foveated --model.foveated-patcher.resolution 64 --model.foveated-patcher.cart-patch-size 8 --model.foveated-patcher.arch-flag doubleres --model.foveated-patcher.conditioning.mode film --model.foveated-patcher.conditioning.film.input position --model.foveated-patcher.conditioning.film.fourier.num-features 256 --model.foveated-patcher.conditioning.film.fourier.sigma 16 --model.foveated-patcher.hidden-dims-patch-embed 768"
# =================
