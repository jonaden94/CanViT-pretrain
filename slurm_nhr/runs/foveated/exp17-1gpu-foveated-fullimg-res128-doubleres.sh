...
# (resolution=128, cart_patch_size=16, arch_flag=doubleres). Otherwise
# identical to exp17-1gpu-foveated-fullimg-res128 (notebook-aligned defaults
# for fov, cmf_a, fixation_size, sampler, sample_cortex).
set -euo pipefail

# === ESSENTIALS (ALWAYS NEED TO BE SPECIFIED) ===
RUN_GROUP=foveated
RUN_NAME=exp17-1gpu-foveated-fullimg-res128-doubleres
...
CFG_LOG_EVERY=512
CFG_NUM_WORKERS=4
EXTRA_ARGS="--model.patcher-name foveated --model.foveated-patcher.resolution 128 --model.foveated-patcher.cart-patch-size 16 --model.foveated-patcher.arch-flag doubleres"
# =================
