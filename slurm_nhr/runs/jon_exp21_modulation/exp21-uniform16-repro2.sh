#!/bin/bash
# Reproduction of exp21-uniform16-repro (uniform, vitb16 = 16px patches, 8x8 grid,
# non-overlapping). Doubles as a regression check that the new overlapping-patch
# feature left the uniform non-overlap path bit-identical (verified: scene_pos
# maxdiff 0). 64 patches + 5 backbone registers (default) = 69 tokens. Precomputed.
set -euo pipefail
...
CFG_LOG_EVERY=512
CFG_NUM_WORKERS=4
EXTRA_ARGS="--model.patcher-name uniform --backbone-name vitb16"
# =================
