#!/bin/bash
# Foveated, derived from exp21-cond4-fovi-film-pos-sigma4-repro; only fov/res/cart changed:
# fov=140, resolution=81, cart_patch_size=10, force=False -> 71 patches.
# 71 patches + 0 backbone registers = 71 backbone tokens (held constant). Precomputed features.
set -euo pipefail

# === ESSENTIALS ===
RUN_GROUP=jon_exp21_modulation
RUN_NAME=exp21-cond4-fovi-fov140-res81
...
CFG_LOG_EVERY=512
CFG_NUM_WORKERS=4
EXTRA_ARGS="--model.patcher-name foveated --model.foveated-patcher.fov 140 --model.foveated-patcher.resolution 81 --model.foveated-patcher.cart-patch-size 10 --model.foveated-patcher.arch-flag doubleres --model.foveated-patcher.no-force-patches-less-than-matched --model.foveated-patcher.conditioning.mode film --model.foveated-patcher.conditioning.film.fourier.num-features 256 --model.foveated-patcher.conditioning.film.fourier.sigma 4 --model.n-backbone-registers 0"
# =================
