#!/bin/bash
# cond4-reg + positional FiLM (fourier 256/sigma4) + per-rollout sampled scale,
# uniform[0.25,1.41]. Scale-AWARE FiLM (encode_scale ON) -> test arm; differs from
# exp21-cond4-reg-film-pos-sigma4-scale-rollout ONLY by --...film.encode-scale.
set -euo pipefail

# === ESSENTIALS ===
RUN_GROUP=jon_exp21_modulation
RUN_NAME=exp21-cond4-reg-film-pos-sigma4-scale-rollout-filmscale
ARRAY=0-48%1                                   # 49 jobs from scratch (49*4096 = 200,704 steps); on-the-fly features + validation ON
TIME=0-01:30:00
MEM=128G
NGPU=1
