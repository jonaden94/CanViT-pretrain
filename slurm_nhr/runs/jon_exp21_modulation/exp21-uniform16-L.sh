#!/bin/bash
# Same as exp21-uniform16-repro (uniform, vitb16, 8x8 grid) but distilling from
# DINOv3 ViT-L (teacher_dim 1024, auto-detected -> prediction heads resize to 1024)
# instead of ViT-B. Requires on-the-fly features (precomputed shards are B-only),
# so it reads the no-features dataset. Student/canvas/registers unchanged.
set -euo pipefail

# === ESSENTIALS ===
RUN_GROUP=jon_exp21_modulation
RUN_NAME=exp21-uniform16-L
...
EXTRA_ARGS="--model.patcher-name uniform --backbone-name vitb16 --webdataset-dir /mnt/lustre-rzg/workspaces/ws/nib00021/u25995-canvit-data-no-features/webdataset-imagenet-1k-no-features"
# =================
