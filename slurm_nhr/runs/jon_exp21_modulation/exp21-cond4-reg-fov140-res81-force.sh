#!/bin/bash
# Regularized (fovi_regularized square) counterpart of exp21-cond4-fovi-fov140-res81-force.
# Same fovi geometry (fov=140, res=81, cart=10), m_override=14, force=True -> 55 patches
# (matches the foveated version exactly). 55 patches + 16 backbone registers = 71. Precomputed features.
set -euo pipefail

# === ESSENTIALS ===
RUN_GROUP=jon_exp21_modulation
RUN_NAME=exp21-cond4-reg-fov140-res81-force
...
CFG_LOG_EVERY=512
CFG_NUM_WORKERS=4
EXTRA_ARGS="--model.patcher-name square --model.square-patcher.method fovi_regularized --model.square-patcher.fov 140 --model.square-patcher.resolution 81 --model.square-patcher.cart-patch-size 10 --model.square-patcher.force-patches-less-than-matched --model.square-patcher.m-override 14 --model.square-patcher.strict-nest-when-possible --model.square-patcher.conditioning.mode film --model.square-patcher.conditioning.film.fourier.num-features 256 --model.square-patcher.conditioning.film.fourier.sigma 4 --model.n-backbone-registers 16"
# =================
