#!/bin/bash
# Strided square patcher, like exp20 cond6 but grid_size_fovea=2, edge=[2,6,9,18], drop-corners.
# 40 patches (44 minus the 4 dropped corners) + 31 backbone registers = 71 backbone tokens
# (held constant). Precomputed features.
set -euo pipefail

# === ESSENTIALS ===
RUN_GROUP=jon_exp21_modulation
RUN_NAME=exp21-strided-gf2
...
CFG_LOG_EVERY=512
CFG_NUM_WORKERS=4
EXTRA_ARGS="--model.patcher-name square --model.square-patcher.method strided --model.square-patcher.grid-size-fovea 2 --model.square-patcher.patch-size 8 --model.square-patcher.edge-length-multipliers 2 6 9 18 --model.square-patcher.drop-corners --model.n-backbone-registers 31"
# =================
