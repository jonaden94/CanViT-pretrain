#!/bin/bash
# cond4-reg + prune inner-ring redundant patches (min_ring_new_pixels=30 -> -13 patches), +13 registers (5->18)
set -euo pipefail

# === ESSENTIALS ===
RUN_GROUP=jon_exp21_modulation
RUN_NAME=exp21-cond4-reg-prune30
ARRAY=0-13%1                                   # 14 remaining of 49 (resume from step 143360); on-the-fly features + validation ON
TIME=0-01:30:00
MEM=128G
NGPU=1
...
CFG_LOG_EVERY=512
CFG_NUM_WORKERS=4
EXTRA_ARGS="--model.patcher-name square --model.square-patcher.method fovi_regularized --model.square-patcher.resolution 64 --model.square-patcher.cart-patch-size 8 --model.square-patcher.no-force-patches-less-than-matched --model.square-patcher.m-override 10 --model.square-patcher.strict-nest-when-possible --model.square-patcher.min-ring-new-pixels 30 --model.n-backbone-registers 18 --webdataset-dir /mnt/lustre-rzg/workspaces/ws/nib00021/u25995-canvit-data-no-features/webdataset-imagenet-1k-no-features"
# =================
